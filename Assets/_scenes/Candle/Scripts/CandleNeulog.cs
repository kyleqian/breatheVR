...
public class CandleNeulog : MonoBehaviour {

    public Light lightSource;
    public ParticleAnimator candleFlame;

    const int MinLightSpotAngle = 30;
    const int MaxLightSpotAngle = 80;

    const float MinLightIntensity = 0.20f;
    const float MaxLightIntensity = 5.0f;

    const float MinFlameSizeGrow = -1.0f;
...
//	private const string Url = "http://localhost:22002/NeuLogAPI?GetSensorValue:[Respiration],[1]";
	...
	void Start() {
        ...
        // Defaults for candle
        lightSource.spotAngle = MinLightSpotAngle;
        lightSource.intensity = MinLightIntensity;
        candleFlame.sizeGrow = MinFlameSizeGrow;
    }
    ...
    void Update() {
        ...
        if (!calibrating) {
            ...
            // Candle behavior
            lightSource.spotAngle = (MaxLightSpotAngle - MinLightSpotAngle) * doubleBoundedPercentageOfMax + MinLightSpotAngle;
            lightSource.intensity = (MaxLightIntensity - MinLightIntensity) * doubleBoundedPercentageOfMax + MinLightIntensity;
            candleFlame.sizeGrow = (MaxFlameSizeGrow - MinFlameSizeGrow) * doubleBoundedPercentageOfMax + MinFlameSizeGrow;
        }
    }
